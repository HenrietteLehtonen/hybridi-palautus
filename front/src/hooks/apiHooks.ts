...
const useMedia = () => {
  ...
  // poista
  const deleteMediabyID = async (media_id: number, token: string) => {
    const options = {
      method: 'DELETE',
      headers: {
        Authorization: 'Bearer ' + token,
  ...
  return {mediaArray, postMedia, deleteMediabyID};
};
...
const useTag = () => {
  ...
  return {getAllTags, getTagsByMediaId, postTag};
};
